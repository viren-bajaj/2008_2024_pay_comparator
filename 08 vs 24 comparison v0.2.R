...
eng_old_pay_scales <- data.frame(
  grade_eng_old = c("FY1","FY1","FY1","FY2","FY2","FY2","StR","StR","StR","StR","StR","StR","StR","StR","StR","StR"),
...
#England pay options	
 ...
 h6(strong("Advanced options for 2008 pay")),
      ...
      h6(strong("Select Grade")),
					      ...
					      h6(strong("Select Incremental Pay")),
					        radioButtons(
							"pay_eng_08", "Within your grade, what incrmental point are you at?",
...
# ========================
	...
	observe({
		if (input.is_advanced == true) {# Determine allowed increments based on grade
...
#GP band overrides LTFT banding, i think (but unclear how it interacts with the F5-F9 stuff), if GP box is ticked, this won't even appear
	observeEvent(input$is_ltft, {
		if(input.is_advanced == true && input$is_ltft) {
...
#in 2010 F!s got 5% banding ate baseline		
	# get ltft mult	
...
# ========================
 ...
 # Add universal items for England 
  ...
  out <- c(out, sprintf("<li>2008 Gross pay adjusted for inflation: %.2f%%</li>", gross_pay_08_infl))
  out <- c(out, sprintf("<li>2008 Net pay adjusted for inflation: %.2f%%</li>", net_08_infl))
  out <- c(out, "</ul>")
